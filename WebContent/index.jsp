		...
		<div>
			<DIV style="width: 330px; height: 228px;" class="tuhuo">
				<A href="" target="_blank"><IMG style="width: 330px; height: 208px;" id="fou_img" src=""></A>
					...
					<c:forEach var="imageNews" items="${imageNewsList}">
						<P style="height: 20px;" class="tc"><A href="news?action=show&newsId=${imageNews.newsId }" target="_blank" title="${imageNews.title }">22${fn:substring(imageNews.title,0,18) }</A></P>
					</c:forEach>
			  <UL>
			  <LI class="fouce">1</LI>
			  <LI>2</LI>
		...
		<div class="newsHeader_list">
			<div class="newsHeader">
				<h3><a href="news?action=show&newsId=${headNews.newsId }" title="${headNews.title }">${fn:substring(headNews.title,0,10) }</a></h3>
					<p>${headNews.title }">${fn:substring(headNews.content,0,40) }...<a href="news?action=show&newsId=${headNews.newsId }" >[查看全文]</a></p>
			</div>
			<div class="currentUpdate">
				<div class="currentUpdateHeader">最新更新
				</div>
	...
	<div class="span3">
		<div class="data_list hotspot_news_list">
			...
			<div class="datas">
				...
				<ul>
			<c:forEach items="${hotSpotNewsList }" var="hotSpotNews">
